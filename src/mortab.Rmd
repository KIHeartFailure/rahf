...
ormy <- function(data, ravar = "sos_com_ra", popname) {
  ...
  nc <- 2 + length(levsef)
  out <- data.frame(matrix(NA, ncol = nc, nrow = 4))
  ...
  # log reg regressions
  ## crude
  mod <- summary(glm(formula(paste0("shf_ef_cat2 == 'LVEF =>40%' ~ ", ravar)),
    family = binomial(link = "logit"),
    data = data
  ))

  out[2, 2] <- "Crude OR (95% CI), p"
  out[2, 3:4] <- c(
    "ref",
    paste0(
  ...
  out[3, 2] <- "Adj age, sex, comorbs OR (95% CI), p"
  out[3, 3:4] <- c(
    "ref",
    paste0(
      fn(exp(mod$coefficients[2, 1]), 1),
  ...
  out[4, 2] <- "Adj age, sex, comorbs, socec OR (95% CI), p"
  out[4, 3:4] <- c(
    "ref",
    paste0(
      fn(exp(mod$coefficients[2, 1]), 1),
...
mormy <- function(data, ravar = "sos_com_ra", popname) {
  ...
  ## crude
  mod <- summary(multinom(formula(paste0("shf_ef_cat ~ ", ravar)), data = data))
